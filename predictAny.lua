...
velocities=inputs[{{},2}]
velocities=velocities:mul(scale_in)
...
gnuplot.plot({'Actual', t, targets1, '-'}, {'Prediction', t, predictions1*2, '-'},{'Angular Velocity', t, velocities,'-'})--, {'Error', t, torch.cat(errors, torch.zeros(N),2), 'filledcurves'})
...
gnuplot.ylabel('Joystick left right deflection')
--gnuplot.title('Noisy Seqence Data Plot')
--gnuplot.title('Sequence Prediction on testSeq2 forward 10 and leftward 5 meters 1(Without Scale Prediction) [MLP-50-50 AR(16) with 10/16s delay ]')
gnuplot.title('Sequence Prediction on testSeq1 forward 10 and leftward 5 meters 1(PredictionX2) [MLP-50-50 AR(16) with 10/16s delay ]')
gnuplot.plotflush()
...
print(criterion:forward(predictions:div(scale_out)*2, targets:div(scale_out)))
...
file1 = io.open("/home/lili/workspace/RNN/data/freq16/pedictedJoystickLR.txt", "w")
file1:write(resultLR)
file1:close()

file2 = io.open("/home/lili/workspace/RNN/data/freq16/pedictedJoystickBF.txt", "w")
